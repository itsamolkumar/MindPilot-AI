...
export function generateCoachResponse(entries = [], analyses = []) {
  ...
  const forecast = forecastMood(sorted);
  ...
  if (!crisis.immediate?.length && riskMap.burnout) {
    immediateActions.push("Take one recovery break before the next study session.");
  }
  ...
  return {
    ...
    confidence,
    crisis,
    commandHelp: Object.keys(COMMAND_RESPONSES).map((command) => ({ command, purpose: COMMAND_RESPONSES[command] })),
  };
}
...
export function parseCoachCommand(command = "", coachData) {
  ...
  switch (normalized) {
    ...
    case "/forecast":
      return {
        title: "Forecast",
        text: `Tomorrow mood forecast: ${coachData.forecast?.tomorrow?.futureMood || "Unknown"}`,
        details: [
